...
import utility.Log;
import farfetch.panda.beta.ios.AppPage.IOS.IOSPageTemplate;
import java.io.File;
import java.net.URL;
 ...
 */
public class IOSPage_Account extends IOSPageTemplate {
    ...
    public IOSPage_Account(IOSDriver theD) throws Exception {
        super(theD);
        this.dr = theD;
    ...
    public IOSPage_Account(IOSDriver theD, StringBuilder p4L) throws Exception {
        super(theD);
        this.dr = theD;
        if (!autoOSType.toString().toLowerCase().contains("windows")) {
            contentFilePath = new StringBuilder(contentFilePath.toString().replaceAll("\\\\", File.separator));
    ...
    // for dry run
    public IOSPage_Account(StringBuilder filename, StringBuilder p4L) throws Exception {
        super(filename, p4L);
        //  this.dr = theD;
        if (!autoOSType.toString().toLowerCase().contains("windows")) {
            contentFilePath = new StringBuilder(contentFilePath.toString().replaceAll("\\\\", File.separator));
    ...
    public IOSPage_Account(String theFile, IOSDriver theD, Log theLogger) throws Exception {
  ...
  //          driver = new IOSDriver(new URL(""), capabilities);
            ...
            IOSPage_Account x = new IOSPage_Account(new StringBuilder("/PageXML/IOS/IOSPage_Account.xml"), null);
