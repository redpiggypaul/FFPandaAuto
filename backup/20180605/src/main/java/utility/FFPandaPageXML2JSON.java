...
public class FFPandaPageXML2JSON {
    ...
    public static void main(String[] args) throws Exception {
        ...
        File folder = new File(new String(System.getProperty("user.dir") + "/PageXML/WEB/FOX/"));// 默认目录
        String keyword = ".xml";
        ...
        for (Iterator it = file2path.entrySet().iterator(); it.hasNext(); ) {
            ...
            elementsMap.clear();
            elementsMap = getElementMapFFPanda(new StringBuilder(fileNameXML));
                ...
                if (root.getName().equalsIgnoreCase("page")) {
                    for (Iterator<?> iter = root.elementIterator(); iter.hasNext(); ) {
                        ...
                        try {
                            ...
                            eLocator = new FFPandaElementEntity(new StringBuilder(root.attributeValue("name").toLowerCase()), elementName, locatorType, locatorValue, dValue, text, tWindowName, sMode);
