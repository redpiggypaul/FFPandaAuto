 ...
 */
public class FFPandaJsonReader {
    ...
    public static HashMap<StringBuilder, StringBuilder> readJSON4defaultValue(String fileName) throws Exception {
        ...
        try {
            ...
            throw new XMLException(FFPandaJsonReader.class.getName() + Thread.currentThread().getStackTrace()[1].getMethodName() + " : " + e.getMessage());
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            return elementsMap;
            ...
            if ((theFileName.toString().toLowerCase().endsWith(object.get("pagename").getAsString().toLowerCase()))) {
                ...
                for (JsonElement jsonElement : languages2) {
                    ...
                    try {
                        ...
                        eLocator = new FFPandaElementEntity(pageName, eleName, locType, locStr, dValue, text, tWin, sMode);
